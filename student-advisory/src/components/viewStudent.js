    ...
    const StudentCard = ({ data }) => (
        ...
        >
            ...
            <div className="flex flex-col items-center px-3 pb-5 gap-1">
                <p className="text-[13px] font-semibold text-gray-800 tracking-wide text-center leading-tight line-clamp-2">
                    {data.stud_name}
                </p>
                <p className="text-[11px] text-gray-400 tracking-widest font-medium">
                    {data.stud_matric}
                </p>
            </div>
            ...
            {open && selectedStudent && (
                ...
                >
                    ...
                    >
                        ...
                        <button
                            onClick={() => { setOpen(false); setSelectedStudent(null); }}
                            className="absolute top-4 right-4 z-10 w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 transition-colors flex items-center justify-center"
                            aria-label="Close"
                        >
                        ...
                        {/* Header */}
                        <div className="bg-gradient-to-br from-green-800 to-green-600 px-6 pt-10 pb-8 flex flex-col items-center gap-3">
                            <div className="rounded-full border-4 border-white/80 shadow-lg overflow-hidden w-[100px] h-[100px]">
                                <Image
                                    src={
                            ...
                            <div className="text-center">
                                ...
                                <span className="inline-block mt-1.5 px-3 py-0.5 rounded-full bg-white/20 text-white/90 text-xs font-medium tracking-widest">
                        ...
                        <div className="px-6 py-2 divide-y divide-gray-100">
                            {infoRows.map(({ icon, label, value }) => (
                                <div key={label} className="flex items-center justify-between py-3.5 text-sm gap-4">
                        ...
                        <div className="px-6 pt-2 pb-6">
                            <button
                                onClick={() => { setOpen(false); setSelectedStudent(null); }}
                                className="w-full py-3 rounded-xl bg-green-700 hover:bg-green-800 active:scale-[0.98] transition-all text-white text-sm font-semibold tracking-wide"
                            >
                                Close
                            </button>
                        </div>
